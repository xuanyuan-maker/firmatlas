...
项目已完成 MVP：支持 TP-Link 中国站和美国站两个相互隔离的数据来源，并通过全部32项 MVP 验收标准。
...
选择具体 Artifact 下载并查看历史：

```bash
uv run firmatlas --data-dir data download <artifact-id>
...
MVP 后的非阻塞优化方向包括：缩短 TP-Link US 全量采集时间、统一历史 Python 文件格式，以及按需支持 TP-Link CN 旧式固件标题。详细需求、验收证据和开发计划作为本地资料维护，不包含在公开仓库中。
