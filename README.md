...
FirmAtlas 是一款实验性质的 IoT 固件目录与按需下载工具，面向漏洞研究者和安全 Agent。
它从厂商公开渠道收集产品、硬件版本、固件发布和下载资源元数据，以 SQLite 保存为可查询目录；只有明确选择
...
- 按需流式下载，按来源声明大小执行带边界的一致性检查，并记录实际大小、SHA-256 和官方校验结果。
